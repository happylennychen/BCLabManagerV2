...
namespace BCLabManager.Model
{
    public class Chroma17200Processer : ITesterProcesser
    {
        ...
        private bool ParseChroFormaV2(string[] inToken, out string outVolt, out string outCurr, out string outTemp, out string outAccm, out string outDate, out UInt32 outSerial)
        {
            ...
            if (inToken.Length > (int)ColumnIndex.ChStatus)
            {
                sStepCtlg = inToken[(int)ColumnIndex.ChStatus];
            ...
            sStepCtlg = inToken[(int)ColumnIndex.ChChgDsg];
            ...
            outDate = new string(inToken[(int)ColumnIndex.ChTime].ToCharArray());
            outCurr = inToken[(int)ColumnIndex.ChCurrent];
            if (iMinus == -1)
            {
            ...
            outVolt = new string(inToken[(int)ColumnIndex.ChVoltage].ToCharArray());
            outTemp = new string(inToken[(int)ColumnIndex.ChTemperature].ToCharArray());
            outAccm = new string(inToken[(int)ColumnIndex.ChAccMah].ToCharArray());
            if (!UInt32.TryParse(inToken[(int)ColumnIndex.ChSerial], out outSerial))
            {
