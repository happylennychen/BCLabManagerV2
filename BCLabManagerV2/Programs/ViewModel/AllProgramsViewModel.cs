        ...
        //相当于Edit，需要修改TestRecord的属性（vm和m层面都要修改），保存到数据库。还需要修改Assets的属性（vm和m层面都要修改），保存到数据库
        {
            ...
            if (evm.IsOK == true)
            {
                ...
                try
                {
                    ...
                    _programService.RecipeService.TestRecordService.Commit(testRecord.Record, evm.Comment, CreateRawDataList(evm.FileList), time[0], time[1],SelectedProgram.Project.Name, SelectedProgram.Name, SelectedRecipe.Name);
                }
                catch
                {
                    _programService.RecipeService.TestRecordService.Commit(testRecord.Record, evm.Comment, CreateRawDataList(evm.FileList), DateTime.MinValue, DateTime.MinValue, SelectedProgram.Project.Name, SelectedProgram.Name, SelectedRecipe.Name);
                }
            }
        }
